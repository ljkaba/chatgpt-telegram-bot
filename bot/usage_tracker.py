...
class UsageTracker:
    ...
    def __init__(self, user_id, user_name, logs_dir="usage_logs"):
        ...
        if os.path.isfile(self.user_file):
            ...
            if "messages" not in self.usage["usage_history"]:
                self.usage["usage_history"]["messages"] = {}
            ...
            for key in ["chat_tokens", "transcription_seconds", "number_images", "tts_characters", "vision_tokens", "messages"]:
                if key not in self.usage["usage_history"]:
                    self.usage["usage_history"][key] = {}
    ...
    def add_chat_tokens(self, tokens, tokens_price=0.002):
        """Ajoute les tokens utilisés à l'historique de l'utilisateur."""
        try:
            today = date.today()
            token_cost = round(float(tokens) * tokens_price / 1000, 6)
            self.add_current_costs(token_cost)
            # Vérifiez et initialisez les clés
            if "usage_history" not in self.usage:
                self.usage["usage_history"] = {}
            if "chat_tokens" not in self.usage["usage_history"]:
                self.usage["usage_history"]["chat_tokens"] = {}
            if str(today) not in self.usage["usage_history"]["chat_tokens"]:
                self.usage["usage_history"]["chat_tokens"][str(today)] = 0
            self.usage["usage_history"]["chat_tokens"][str(today)] += tokens
            with open(self.user_file, "w") as outfile:
                json.dump(self.usage, outfile)
        except Exception as e:
            logging.error(f"Error in add_chat_tokens for user {self.user_id}: Type={type(e).__name__}, Message={str(e)}")
            raise
    ...
    def add_message(self, plan: str):
        """Ajoute un message au suivi pour les utilisateurs gratuits."""
        if plan != "free":  # Ne compter que pour les utilisateurs gratuits
            return
        today = date.today()
        if str(today) not in self.usage["usage_history"]["messages"]:
            self.usage["usage_history"]["messages"][str(today)] = 0
        self.usage["usage_history"]["messages"][str(today)] += 1
        with open(self.user_file, "w") as outfile:
            json.dump(self.usage, outfile)

    def get_current_message_count(self):
        """Retourne le nombre de messages pour aujourd'hui."""
        today = date.today()
        return self.usage["usage_history"]["messages"].get(str(today), 0)
    ...
    def get_current_tts_usage(self):
        ...
        for tts_model in tts_models:
            if (
                ...
                and str(today)
                in self.usage["usage_history"]["tts_characters"][tts_model]
            ):
                characters_day += self.usage["usage_history"]["tts_characters"][
                    tts_model
                ][str(today)]
        ...
        for tts_model in tts_models:
            if tts_model in self.usage["usage_history"]["tts_characters"]:
                for today, characters in self.usage["usage_history"]["tts_characters"][
                    tts_model
                ].items():
                    if today.startswith(month):
    ...
    def add_current_costs(self, request_cost):
        ...
        self.usage["current_cost"]["all_time"] = (
            self.usage["current_cost"].get("all_time", self.initialize_all_time_cost())
            + request_cost
        )
    ...
    def get_current_transcription_duration(self):
        ...
        today = date.today()
        if str(today) in self.usage["usage_history"]["transcription_seconds"]:
            seconds_day = self.usage["usage_history"]["transcription_seconds"][
                str(today)
            ]
        else:
            seconds_day = 0
    ...
    def get_current_cost(self):
        ...
        if today == last_update:
            ...
            if today.month == last_update.month:
                ...
                cost_month = 0.0
        # add to all_time cost, initialize with calculation of total_cost if key doesn't exist
        cost_all_time = self.usage["current_cost"].get(
            "all_time", self.initialize_all_time_cost()
        )
        return {
            "cost_today": cost_day,
    ...
    def initialize_all_time_cost(
        ...
        total_images = [
            sum(values)
            for values in zip(*self.usage["usage_history"]["number_images"].values())
        ]
        ...
        total_transcription_seconds = sum(
            self.usage["usage_history"]["transcription_seconds"].values()
        )
        ...
        total_characters = [
            sum(tts_model.values())
            for tts_model in self.usage["usage_history"]["tts_characters"].values()
        ]
        ...
        tts_cost = round(
            sum(
                [
                    count * price / 1000
                    for count, price in zip(total_characters, tts_prices_list)
                ]
            ),
            2,
        )
        ...
        all_time_cost = (
            token_cost + transcription_cost + image_cost + vision_cost + tts_cost
        )
